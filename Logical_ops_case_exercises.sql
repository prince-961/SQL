...
SELECT 1 IN (5,3) || 9 BETWEEN 8 AND 10; ## 1 is not in (5,3) so False

Select title, released_year books From books where released_year<1980;
SELECT title, author_lname FROM books WHERE author_lname IN ('eggers', 'chabon');
SELECT * FROM books WHERE author_lname = 'lahiri' && released_year > 2000;
SELECT title, pages FROM books WHERE pages BETWEEN 100 AND 200;
SELECT title, author_lname WHERE author_lname LIKE 'c%' || author_lname LIKE 's%';
